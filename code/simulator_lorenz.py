...
class Lorenz:
    def __init__(self, dt=0.01, output_mode=None, params_simulator=None, n_horizon=10, u_weight=1e-3,
        ...
        ex: output_mode = ('x', 'y' 'z') # to output states x & y & z
        ...
        self.x0 = FixedKeysDict(x=0.0,  # convective flow
                                y=0.0,  # temperature variation
                                z=0.0,  # temperature gradient
                                sigma=1.0,  # Prandtl number, represents ratio of fluid viscosity to thermal diffusivity
                                rho=1.0,  # Rayleigh number, describes temperature difference driving convection
                                beta=1.0  # geometric factor related to the physical dimensions of the convection layer
                                )
        ...
        self.u = FixedKeysDict(u_x=np.zeros(j),  # x input
                               u_y=np.zeros(j),  # y input
                               u_z=np.zeros(j)   # z input
                               )
        ...
        # Set output mode
        self.output_mode = ('x', 'y', 'z', 'sigma', 'rho', 'beta')  # default output mode
        ...
        x = self.model.set_variable(var_type='_x', var_name='x', shape=(1, 1))  # convective flow
        y = self.model.set_variable(var_type='_x', var_name='y', shape=(1, 1))  # temperature variation
        z = self.model.set_variable(var_type='_x', var_name='z', shape=(1, 1))  # temperature gradient
        sigma = self.model.set_variable(var_type='_x', var_name='sigma', shape=(1, 1))  # Prandtl number
        rho = self.model.set_variable(var_type='_x', var_name='rho', shape=(1, 1))  # Rayleigh number
        beta = self.model.set_variable(var_type='_x', var_name='beta', shape=(1, 1))  # geometric factor

        # Define input variables
        u_x = self.model.set_variable(var_type='_u', var_name='u_x', shape=(1, 1))  # x input
        u_y = self.model.set_variable(var_type='_u', var_name='u_y', shape=(1, 1))  # y input
        u_z = self.model.set_variable(var_type='_u', var_name='u_z', shape=(1, 1))  # z input

        # Define dynamics equations
        self.model.set_rhs('x', sigma*(y - x) + u_x)
        self.model.set_rhs('y', x*(rho - z) - y + u_y)
        self.model.set_rhs('z', x*y - beta*z + u_z)
        self.model.set_rhs('sigma', x * 0)
        self.model.set_rhs('rho', x * 0)
        self.model.set_rhs('beta', x * 0)
    ...
    def calculate_simulation_data(self):
        ...
        # Add other variables
        # self.sim_data['x_y'] = self.sim_data['x'] / self.sim_data['y']
        self.sim_data['xy'] = self.sim_data['x'] * self.sim_data['y']
        self.sim_data['xz'] = self.sim_data['x'] * self.sim_data['z']
        self.sim_data['yz'] = self.sim_data['y'] * self.sim_data['z']
    ...
    def set_mpc_objective(self, u_weight=1e-3):
        ...
        self.mpc.set_rterm(u_x=u_weight, u_y=u_weight, u_z=u_weight)  # input penalty
